package com.example.gateway.grpc.service
 ...
 */
@GrpcService
class AuthService(private val claimsUtils: ClaimsUtils, private val userRepository: UserRepository) :
    ...
    override fun getUser(request: UserRequest, responseObserver: StreamObserver<UserResponse?>) {
        Mono.just(if (request.jwt.isNotBlank()) claimsUtils.extractAllClaims(request.jwt).id else request.id)
